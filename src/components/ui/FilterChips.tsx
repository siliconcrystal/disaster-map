...
export function FilterDropdown({
  ...
  const [tempDateTS, setTempDateTS] = useState<{ start: number | null; end: number | null }>({
    start: null,
    end: null,
  });
  ...
  const handleSelect = (val: string) => {
    if (requiresAuthValues.includes(val) && !isLoggedIn) {
  ...
  return (
    <>
      ...
      >
        <span className="tracking-wide">{value === 'all' ? label : displayLabel}</span>
        <ChevronDown
          ...
          <>
            ...
            <div
              ...
              style={{
                ...
                left: Math.min(
                  position.left,
                  typeof window !== 'undefined' ? window.innerWidth - 200 : position.left,
                ),
              }}
...
export function FilterChips() {
  ...
  const { setAdvancedFilterOpen, currentDisasterType } = useUIStore();

  // 根據當前災害類型動態取得任務類型選項
  const typeOptions = useMemo(
    () => getTaskTypeFilterOptions(currentDisasterType),
    [currentDisasterType],
  );
